  ...
  >
    <el-card>
      ...
      >
        ...
        <el-table-column label="退单号" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{ scope.row.outRefundNo }}</span>
          </template>
        </el-table-column>
        <el-table-column label="退款金额" header-align="center" align="right" show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-table-column label="交易状态" align="center" width="120">
          <template slot-scope="scope">
            <el-tag>{{ filterTradeStatus[scope.row.refundStatus] }}</el-tag>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    init(order, filterTradeStatus, filterPayMode) {
      ...
      this.dataForm.outRefundNo = order.out_refund_no
      ...
      this.$nextTick(() => {
        if (this.dataForm.outTradeNo) {
          const fetchData = { outTradeNo: this.dataForm.outTradeNo, outRefundNo: this.dataForm.outRefundNo }
          ...
          api.payRefundTrade.hisRefund({ data: fetchData }).then(res => {
            if (res && res.data && res.data.success) {
    ...
    checkSuccess() {
      const fetchData = {
        outTradeNo: this.hisOrder[0].outTradeNo, outRefundNo: this.dataForm.outRefundNo
      }

      api.payRefundTrade.hisRefundCheckSuccess({ data: fetchData }).then(res => {
        if (res && res.data && res.data.success) {
          this.$message({
            message: '操作成功',
            type: 'success',
