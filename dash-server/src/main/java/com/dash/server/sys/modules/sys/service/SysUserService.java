...
public interface SysUserService extends IService<SysUserEntity> {
	...
	PageUtils list(SysUserDTO sysUser);

	void saveUpdate(SysUserDTO user);

	SysUserEntity infos(SysUserDTO user);

	void resetPassword(SysUserDTO user);

	Map<String, Object> validatorRepeat(SysUserDTO user);

	void delete(Long userId);

	void passwordUpdate(SysUserDTO user);
}
